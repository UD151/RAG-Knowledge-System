...
#     print(f"\n模型回答: {answer}")
#     return {"message": "回答： "+ answer}

@app.post("/api/stream")
async def stream_llm_response(req: ChatRequest):
    query =req.query
    retrieved_doc_ids,retrieved_chunks = retrieve_docs(query,index)
    context = "\n".join(retrieved_chunks)
    stream = await async_client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "相关信息如下："+context+"你是一个知识库助手，请根据所给信息严格回答问题，可适当延伸，但不要偏离主题，不要虚构内容。不要说根据某某，因为用户看不到上下文知识。不要说根据所给信息或者类似的开头！直接回答即可\n"},
